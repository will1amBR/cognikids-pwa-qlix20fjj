...
export const ConfirmEmailChangePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const confirm = async () => {
      try {
        ...
        // User must sign in with their new email
        pb.authStore.clear()
        setIsSuccess(true)
      } catch (err: any) {
        setError('O token de alteração de email expirou ou é inválido.')
      } finally {
        setIsLoading(false)
      }
    }
  ...
  return (
    <AuthLayout title="Alteração de email" subtitle="Atualização de dados cadastrais">
      {isLoading ? (
        <div className="py-8 flex flex-col items-center justify-center gap-3">
          <Loader2 className="w-8 h-8 text-orange-500 animate-spin" />
          <p className="text-sm font-semibold text-slate-600">Confirmando alteração de email…</p>
        </div>
      ) : isSuccess ? (
        <div className="space-y-4 text-center">
          <div className="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-2xl flex items-center gap-3 text-left text-xs">
            ...
            <div>
              <p className="font-bold">Email atualizado com sucesso! 🎉</p>
              <p className="text-emerald-700 mt-0.5">
                Faça login novamente utilizando seu novo endereço de email.
              </p>
            </div>
          </div>
          <Link to="/login">
        ...
        <div className="space-y-4 text-center">
          <div className="p-4 bg-rose-50 border border-rose-200 text-rose-800 rounded-2xl flex items-start gap-3 text-left text-xs">
            ...
            <div>
              <p className="font-bold">Não foi possível alterar</p>
